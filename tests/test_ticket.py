...
def test_get_ticket_by_id(app_with_database):
    ...
    res = app_with_database.get('/tickets/116')
    assert res.status_code == 200
...
def test_delete_ticket(app_with_database, flask_login_admin):
    ...
    res = app_with_database.delete('/tickets/116', headers=flask_login_admin)
    assert res.status_code == 204


def test_delete_ticket_error(app_with_database, flask_login):
    res = app_with_database.delete('/tickets/25', headers=flask_login)
    assert res.status_code == 404
...
#     res = data_admin.put("/tickets/116",
#                          json={
